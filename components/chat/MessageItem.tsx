...
import { ImageWithVoiceCard } from './ImageWithVoiceCard';
...
const MessageItem = React.memo(({
    ...
    if (m.type === 'image_with_voice') {
        const loading = !!m.metadata?.imageGen?.loading;
        if (loading) {
            return commonLayout(
                <div className="flex flex-col items-center justify-center w-[200px] h-[200px] rounded-2xl bg-slate-100 border border-slate-200/80">
                    ...
                    <span className="text-[11px] text-slate-500 font-medium">图片传输中…</span>
                </div>
            );
        }
        return commonLayout(
            <ImageWithVoiceCard message={m} />
        );
    }
    ...
    return prev.msg.id === next.msg.id &&
           ...
           prev.voiceTtsReady === next.voiceTtsReady;
});
