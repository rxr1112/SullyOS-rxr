...
import {
    ...
    generateImage,
    deliverImageWithVoiceMessage,
    createImageLoadingPlaceholder,
    evaluateAutoImageDecision,
    countImagesInTurn,
    generateImageVoiceCaption,
...
const Chat: React.FC = () => {
    ...
    const turnStartUserMsgIdRef = useRef<number | null>(null);
    const lastAutoImageTurnRef = useRef<number | null>(null);
    const prevIsTypingForImageRef = useRef(false);
    ...
    const handlePlayVoice = (msgId: number) => {
        ...
        if (msg?.type === 'image_with_voice') {
            void handleImageVoiceTts(msg);
            return;
        }
    ...
    const handleImageVoiceTts = async (msg: Message) => {
        const existing = voiceDataMap[msg.id];
        if (existing) {
            if (!chatAudioRef.current) chatAudioRef.current = new Audio();
            const audio = chatAudioRef.current;
        ...
        const voiceText = msg.metadata?.voiceInfo?.text?.trim();
        if (!voiceText) {
            addToast('这条图片没有语音解说', 'info');
            return;
        }
        if (!isMinimaxReady()) {
            addToast('MiniMax TTS 未配置，无法播放语音', 'info');
            return;
        }
        setVoiceLoading(prev => new Set(prev).add(msg.id));
        try {
            const spokenText = cleanTextForTts(voiceText);
            const { url: blobUrl, blob } = await synthesizeSpeechDetailed(spokenText, char, apiConfig, {
                languageBoost: char.chatVoiceLang || undefined,
                groupId: apiConfig.minimaxGroupId || undefined,
            });
            if (blobUrl.startsWith('blob:')) voiceBlobUrlsRef.current.add(blobUrl);
            const assetKey = msg.metadata?.voiceInfo?.audioAssetKey || voiceAssetKey(msg.id);
            setVoiceDataMap(prev => ({ ...prev, [msg.id]: { url: blobUrl, originalText: voiceText, spokenText: voiceText } }));
            await persistVoice(msg.id, blobUrl, blob, voiceText, voiceText, char.chatVoiceLang || undefined);
            await DB.updateMessageMetadata(msg.id, (prev: any) => ({
                ...prev,
                voiceInfo: { ...(prev?.voiceInfo || {}), text: voiceText, audioAssetKey: assetKey },
            }));
            if (!chatAudioRef.current) chatAudioRef.current = new Audio();
            chatAudioRef.current.src = blobUrl;
            chatAudioRef.current.onended = () => setPlayingMsgId(null);
    ...
    const runImageGeneration = useCallback(async (
        scenePrompt: string,
        source: 'manual' | 'auto',
        prefsOverride?: ImageGenChatPrefs,
        targetCharId?: string,
    ) => {
        const charId = targetCharId || activeCharacterId;
        const targetChar = characters.find(c => c.id === charId) || charRef.current;
        if (!targetChar || !charId || imageGenBusyRef.current) return;

        const cfg = resolveImageGenConfig(apiConfig.imageGen);
        if (!cfg.enabled) {
            if (source === 'manual') addToast('发图功能未启用，请先在设置中开启', 'info');
            return;
        }

        const prefs = prefsOverride || loadImageGenChatPrefs(charId);
        imageGenBusyRef.current = true;
        setImageGenBusy(true);
        let placeholderId: number | undefined;
        try {
            placeholderId = await createImageLoadingPlaceholder(charId, source);
            ...
            await deliverImageWithVoiceMessage({
                ...
                existingMemories: targetChar.memories,
                onMemoryUpdate: (memories) => updateCharacter(targetChar.id, { memories }),
                placeholderMsgId: placeholderId,
            });
            ...
            addToast(`发图失败: ${e?.message || String(e)}`, 'error');
        } finally {
            imageGenBusyRef.current = false;
            setImageGenBusy(false);
        }
    }, [activeCharacterId, characters, apiConfig, userProfile, addToast, updateCharacter, reloadMessages]);
    ...
    useEffect(() => {
        ...
        window.addEventListener('chat-image-gen-delivered', handler);
        return () => window.removeEventListener('chat-image-gen-delivered', handler);
    }, [reloadMessages]);

    useEffect(() => {
        turnStartUserMsgIdRef.current = null;
        lastAutoImageTurnRef.current = null;
    }, [activeCharacterId]);

    useEffect(() => {
        const wasTyping = prevIsTypingForImageRef.current;
        prevIsTypingForImageRef.current = isTyping;
        if (!wasTyping || isTyping || !char) return;

        const cfg = resolveImageGenConfig(apiConfig.imageGen);
        if (!cfg.enabled || !cfg.autoTrigger || imageGenBusyRef.current) return;

        void (async () => {
            await new Promise(r => setTimeout(r, 300));
            if (activeCharIdRef.current !== char.id || imageGenBusyRef.current) return;

            const recent = await DB.getRecentMessagesByCharId(char.id, 30);
            const lastUser = [...recent].reverse().find(m => m.role === 'user');
            if (!lastUser) return;
            if (lastAutoImageTurnRef.current === lastUser.id) return;

            const assistantTexts = recent
                .filter(m => m.timestamp >= lastUser.timestamp && m.role === 'assistant' && m.type === 'text')
                .map(m => m.content)
                .join('\n');
            if (!assistantTexts.trim()) return;

            if (countImagesInTurn(recent, lastUser.id) >= 1) {
                lastAutoImageTurnRef.current = lastUser.id;
                return;
            }

            const decision = await evaluateAutoImageDecision({
                apiConfig,
                char,
                recentMessages: recent,
                assistantReply: assistantTexts,
            });
            lastAutoImageTurnRef.current = lastUser.id;
            if (!decision.shouldSend || !decision.prompt.trim()) return;

            await runImageGeneration(decision.prompt.trim(), 'auto', undefined, char.id);
        })();
    }, [isTyping, char, apiConfig.imageGen, apiConfig, runImageGeneration]);
    ...
    const handleSendText = async (customContent?: string, customType?: MessageType, metadata?: any) => {
        ...
        turnStartUserMsgIdRef.current = savedUserMsgId;
        lastAutoImageTurnRef.current = null;
    ...
    return (
        ...
        >
            ...
            {char && (
                <ImageGenPanel
                    open={showImageGenPanel}
                    onClose={() => setShowImageGenPanel(false)}
                    charId={char.id}
                    charName={char.name}
                    imageGenEnabled={resolveImageGenConfig(apiConfig.imageGen).enabled}
                    addToast={addToast}
                    busy={imageGenBusy}
                    onGenerate={(prompt: string, prefs: ImageGenChatPrefs) => {
                        setShowImageGenPanel(false);
                        void runImageGeneration(prompt, 'manual', prefs, char.id);
                    }}
                />
            )}
