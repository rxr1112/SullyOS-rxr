...
const WorldHomeApp: React.FC<{ embedded?: boolean; onFullscreen?: (full: boolean) => void }> = ({ embedded, onFullscreen }) => {
    ...
    return (
        <div className="h-full w-full flex flex-col" style={{ background: pageBg }}>
            <GameStyles />
            {/* 顶栏：内嵌进「小小窝」时，列表页不显示顶栏（外层已有）；进世界时显示顶栏 */}
            {embedded && view === 'world' && (
                <div className="relative z-10 px-6 shrink-0" style={{ paddingTop: 'max(3rem, var(--safe-top))' }}>
                    <button onClick={goBack} className="absolute left-4 p-2 rounded-full active:scale-90 transition-all text-purple-300 hover:text-purple-500" style={{ top: 'max(3rem, var(--safe-top))' }}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" /></svg>
                    </button>
                    <div className="text-center">
                        <h1 className="text-[26px] tracking-[0.15em]" style={{ fontFamily: "'Noto Serif SC',serif", color: '#6a5790', textShadow: '0 2px 18px rgba(170,150,220,.4)' }}>{headerTitle}</h1>
                        <div className="flex items-center justify-center gap-2 mt-1.5">
                            <span className="h-px w-10" style={{ background: 'linear-gradient(90deg,transparent,rgba(150,120,190,.5))' }} />
                            <span className="text-[9px] tracking-[0.45em] font-bold text-purple-400/70">✦ HOME ✦</span>
                            <span className="h-px w-10" style={{ background: 'linear-gradient(270deg,transparent,rgba(150,120,190,.5))' }} />
                        </div>
                    </div>
                </div>
            )}
            {!embedded && (
            <div className="shrink-0 sticky top-0 z-10" style={{ background: headerBg, paddingTop: 'var(--safe-top)' }}>
                    ...
                    <div className="relative rounded-3xl overflow-hidden p-5 shadow-[0_10px_30px_rgba(120,100,180,.25)]" style={{ background: 'linear-gradient(150deg,#8e83c4 0%,#a99fd6 52%,#c3c9ea 100%)' }}>
                        ...
                        {embedded && (
                            <div className="absolute top-4 right-4 flex items-center gap-0.5 z-10">
                                <button onClick={() => setShowApiSettings(true)} className="p-2 rounded-full bg-white/20 hover:bg-white/30 active:scale-90 transition-all" title="家园 API 设置">
                                    <GearSix size={18} weight="bold" className="text-white" />
                                </button>
                                <button onClick={startCreate} className="p-2 rounded-full bg-white/20 hover:bg-white/30 active:scale-90 transition-all">
